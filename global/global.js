...
//设备字典
let DeviceDictionary = { //灭菌设备 = 0,清洗设备 = 1,烘乾机 = 2
    0: "灭菌设备",
    1: "清洗设备",
    2: "烘干机"
}
...
function Verification(obj) {
    ...
    switch (obj.type) {
        ...
        case "ArrayNotEmpty": //数组不能为空
            return obj.val == "" ? error : correct;
            // case "ArrayNotAllEmpty": //多个数组不能同时为空
            //     obj.result = [];
            //     obj.val.forEach(item => {
            //         if (item == "") {
            //             obj.result.push(false);
            //         } else {
            //             obj.result.push(true);
            //         }
            //     });
            //     return eval(obj.result.join('+')) == 0 ? error : correct;
        default:
...
function VerificationHandle(arr) {
    for (let i = 0; i < arr.length; i++) {
        if (Verification(arr[i]).bool == false) {
            TipsMsg.$message({
                type: 'error',
                message: arr[i].msg,
                duration: 2000,
