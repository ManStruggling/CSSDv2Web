...
export default {
  ...
  methods: {
    ...
    collapseChange(index) {
      if (
        index != "" &&
        (this.recordList[index].SterilizeTasks == "" ||
          this.recordList[index].SterilizeTasks === null)
      ) {
        axios({
    ...
    searchRecordsData() {
      this.GLOBAL.searchRecord(
        `/api/Sterilize/BiologicalRecallRecords/${this.search_date[0]}/${this.search_date[1]}`,
        this
      );
    }
  },
