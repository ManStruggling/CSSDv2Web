                  ...
                  <div class="tab_content table_unExpand">
                    ...
                    <ul class="clear_float">
                      ...
                      <li>
                        <p>质量检测结果</p>
                        <el-select
                          v-model="item[item.ReviewType]"
                          class="green24x13"
                          @change="((val)=>{reviewResultChange(val,item)})"
                        >
                    ...
                    <h6>
                      <el-button
                        ...
                        class="btn120x40"
                        :disabled="item.ChemicalReviewStatus==3"
                      >新增不合格包</el-button>
                      <el-button
                        v-if="UserInfo.Configuration.IsActiveNotBarCodeProduct"
                        @click="addFailedCountPackage(index)"
                        :disabled="item.ChemicalReviewStatus==3"
                      >新增计数包</el-button>
                    </h6>
...
export default {
  ...
  methods: {
    ...
    packageData2father(data) {
      ...
      if (
        data &&
        this.sterilizeTask.PendingReview[data.index].ChemicalReviewStatus == 2
      ) {
    ...
    handleBarCode(msg) {
      if (/^MJR/.test(msg)) {
        ...
        if (this.sterilizeTask.PendingReview[this.pendingReviewActiveName]) {
          ...
          if (onOff) {
            axios({
              url: `/api/Scanner/SterilizerReviewFailed/${
                this.sterilizeTask.PendingReview[this.pendingReviewActiveName]
                  .SterilizeTaskId
              }/${msg}`
            })
              .then(res => {
