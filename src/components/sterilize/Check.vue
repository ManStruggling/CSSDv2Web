...
<div class="cssd_box" id="sterilizeCheck">
                ...
                <dl class="isActive">
                    ...
                    <dd class="reviewDd" style="display:block;">
                        <el-tabs :tab-position="'left'" :activeName="pendingReviewActiveName" @tab-click="tabClick">
                            <el-tab-pane v-for="(item,index) in sterilizeTask.PendingReview" :key="index" :label="item.SterilizeTaskBarCode" :name="index+''">
                                <div class="tab_content table_unExpand">
                                    ...
                                    <ul class="clear_float">
                                        ...
                                        <li>
                                            <p>质量检测结果</p>
                                            <el-select v-model="item[item.ReviewType]" class="green24x13" @change="((val)=>{reviewResultChange(val,item)})">
                                        ...
                                        <el-table-column label="不合格原因" width="210">
                                            <template slot-scope="props">
                                                <el-select v-model="props.row.FailedCauseId" class="green24x13">
                                                    <el-option v-for="(val,idx) in failedCauses" :key="idx" :label="val.FailedCause" :value="val.FailedCauseId"></el-option>
                                                </el-select>
                                            </template>
                                        </el-table-column>
                ...
                <dl>
                    ...
                    <dd class="reviewDd">
                        <el-tabs :tab-position="'left'" :activeName="sterilizingActiveName">
                            <el-tab-pane v-for="(item,index) in sterilizeTask.Sterilizing" :key="index" :label="item.SterilizeTaskBarCode" :name="index+''">
    ...
    <transition name="fade" enter-active-class="animated fadeIn faster" leave-active-class="animated fadeOut faster">
        <!-- 手工录入 -->
        <ManualEnter v-if="isShowManualEnter" :task_index="task_index" @to-father="packageData2father" :ApiUrl="`/api/Scanner/SterilizerReviewFailed/${sterilizeTask.PendingReview[task_index].SterilizeTaskId}`" :BarCodeList="sterilizeTask.PendingReview[task_index].SterilizeFailedPackages"></ManualEnter>
    </transition>
    <transition name="fade" enter-active-class="animated fadeIn faster" leave-active-class="animated fadeOut faster">
        <!-- 失败的计数包 -->
        <FailedCountPackages v-if="isShowFailedCountPackages" :task_index="task_index" @failedCountNumber-to-father="failedCountPackagesData2father" :ApiUrl="`/api/Sterilize/SterilizeNotBarCodePackage/${sterilizeTask.PendingReview[task_index].SterilizeTaskId}`"></FailedCountPackages>
    </transition>
    <!-- 照片 -->
    <transition name="fade" enter-active-class="animated fadeIn faster" leave-active-class="animated fadeOut faster">
        <!-- 拍照 -->
...
export default {
    ...
    created() {
        ...
        axios({
                url: "/api/Sterilize/GetSterilizeReviewTask"
            })
            .then(res => {
    ...
    methods: {
        ...
        //审核结果change
        reviewResultChange(val, item) {
            if (val == 3) {
                item.SterilizeFailedPackages = [];
            }
        },
        //添加不合格包
        ...
        packageData2father(data) {
            ...
            if (data && this.sterilizeTask.PendingReview[data.index].ChemicalReviewStatus == 2) {
                // data.data.Package.FailedCauseId=this.failedCauses[0].FailedCauseId;
                this.sterilizeTask.PendingReview[data.index].SterilizeFailedPackages.push(data.data.Package);
            }
        },
        //失败的计数包数据传递
