            ...
            <div class="collapseTd">
              <p>
                <el-button
                  ...
                  :disabled="UserInfo.JobAndCompetence.includes('111')||UserInfo.JobAndCompetence.includes('000')||UserInfo.JobAndCompetence.includes('100')?false:true"
                >修改</el-button>
              </p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    collapseChange(index) {
      if (
        index != "" &&
        (this.recordList[index].OuterPackages == "" ||
          this.recordList[index].OuterPackages === null)
      ) {
        axios({
    ...
    searchRecordsData() {
      this.GLOBAL.searchRecord(
        `/api/Recycle/OuterProductRegisterRecords/${this.search_date[0]}/${this.search_date[1]}`,
        this
      );
    }
  }
