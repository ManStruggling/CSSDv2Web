        ...
        <el-collapse accordion @change="collapseChange">
            ...
            <el-collapse-item v-for="(item,collapseIndex) in recordList" :key="collapseIndex" :name="collapseIndex+''">
                ...
                <el-tabs activeName="0" v-show="item.Locations==''?false:true">
                    <el-tab-pane v-for="(value,index) in item.Locations" :key="index" :label="value.RouteName" :name="index+''">
...
export default {
    ...
    methods: {
        ...
        collapseChange(index) {
            if (index != '' && (this.recordList[index].Locations == '' || this.recordList[index].Locations === null)) {
                axios({
        ...
        searchRecordsData() {
            axios({
                url: `/api/Logistics/DriverScheduleRecord/${this.search_date[0]}/${this.search_date[1]}`
            }).then(res => {
                if (res.data.Code == 200) {
