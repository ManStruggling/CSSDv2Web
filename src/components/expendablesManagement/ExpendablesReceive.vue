        ...
        <div class="cssd_title_right">
            <p>
                ...
                <el-select v-model="submitData.RequestPersonId" filterable class="white24x13">
                    <el-option v-for="(item,index) in staffs" :key="index" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </p>
        </div>
    </div>
        ...
        <div class="table_box">
            <el-table :data="submitData.Products">
                ...
                <el-table-column width="210" label="本次领用数">
                    <template slot-scope="props">
                        <el-input-number v-model="props.row.ThisTimeRequestCount" :controls="false" :min="0" :max="props.row.RemainInventoryCount" @change="((newValue,oldValue)=>{handleNumberChange(newValue,oldValue,props.row)})"></el-input-number>
                    </template>
                </el-table-column>
    ...
    methods: {
        ...
        submitComplete() {
            ...
            if (this.GLOBAL.VerificationHandle([{
                ...
                if (this.submitData.Products.every(item => item.Id != "") && this.submitData.Products.every(item => item.ThisTimeRequestCount != 0)) {
                    axios({
