  ...
  <div id="editBox" class="producerBox">
    <div class="editContainer">
      <div class="editContent">
        ...
        <div
          class="RelationBox table_collapse table_unExpand"
          v-if="editBoxData.Type == 0?true:false"
        >
                ...
                <el-table-column label="程序运作时长（分钟）" width="210">
                  <template slot-scope="props">
                    <el-input-number
                      ...
                      :max="300"
                      :controls="false"
                      @change="((newValue,oldValue)=>{numberChange(newValue,oldValue,index,props.$index)})"
                    ></el-input-number>
                  </template>
                </el-table-column>
  ...
  methods: {
    ...
    editBoxSave() {
      ...
      if (
        ...
        if (
          this.GLOBAL.VerificationHandle([
            { val: val_arr1, type: "StringAllNotEmpty", msg: msg_arr1 },
            { val: val_arr2, type: "ArrayAllNotEmpty", msg: msg_arr2 }
          ])
        ) {
          let val_arr3 = [];
          let msg_arr3 = [];
          for (let i = 0; i < this.editBoxData.DeviceModel.length; i++) {
            for (
              let j = 0;
              j < this.editBoxData.DeviceModel[i].DevicePrograms.length;
              j++
            ) {
              val_arr3.push(
    ...
    //新增editBox tr
    addEditBoxTr() {
      this.editBoxData.DeviceModel.push({ Name: "", DevicePrograms: [] });
    }
  }
};
</script>
