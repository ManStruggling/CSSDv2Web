...
export default {
  ...
  methods: {
    ...
    fileChange(evt) {
      ...
      reader.onload = event=>{
        ...
        for(let i=0;i<objData.Sheet1.length;i++){
          objData.Sheet1[i].RolesForStaff=objData.RolesForStaff.filter(val=>{return val.Id===objData.Sheet1[i].Id});
        }
    ...
    requestData(arr,index){
      axios({url:"/api/Staff",method:"POST",data:arr[index]}).then(res=>{
    ...
    //搜素
    searchThisTableData() {
        this.baseDataSearch(`/odata/staffs?$filter=contains(name,${"'"+this.basic_search+"'"})`);
    },
    //新增tr
    addTableTr() {
      this.toChildData = {
