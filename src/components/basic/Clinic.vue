<template>
  ...
  <div class="basic_main">
    <div class="basic_main_head">
      ...
      <p>
        <a href="\ImportBaseData\科室汇入.xlsx" download="科室汇入" v-if="GLOBAL.UserInfo.JobAndCompetence.includes('000')">下载模板</a>
    ...
    <transition
      ...
      leave-active-class="animated fadeOut faster"
    >
      <ClinicBox v-if="showEditBox" @to-father="child2father" :data="toChildData"></ClinicBox>
    </transition>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    fileChange(evt) {
      ...
      reader.onload = event=> {
        ...
        for(let i=0;i<objData.Sheet1.length;i++){
          objData.Sheet1[i].SubClinics=objData.SubClinics.filter(val=>{return val.ClinicId===objData.Sheet1[i].Id});
        }
    ...
    requestData(arr,index){
      axios({url:"/api/Clinic",method:"POST",data:arr[index]}).then(res=>{
        if(res.data.Code==200){
          this.table_data=res.data.Data;
