    ...
    <div class="editContainer">
      <div class="editContent">
        <ul class="clear_float">
          <li>
            <p>员工姓名</p>
            <el-input type="text" placeholder="员工姓名(必填)" v-model.trim="editBoxData.Name" maxlength="10"></el-input>
          </li>
          <li>
            <p>员工工号</p>
        ...
        <el-tabs v-model="tabActiveName" closable addable type="card" @edit="handleTabsEdit">
          <el-tab-pane
            v-for="(item,index) in editBoxData.RolesForStaff"
            :key="index"
            ...
            <div class="powerDetail">
              <el-table :data="item.JobAndCompetence">
                <el-table-column label="岗位">
                  <template slot-scope="props">
                    <el-select
                      v-model="props.row.RoleJob"
                      filterable
                      default-first-option
                      @change="((val)=>{forbidOption(val,index,props.row)})"
                      :disabled="props.row.Forbid"
                      class="green18x10"
                      placeholder="请选择(必填)"
                    >
                      <el-option
                        v-for="(val,idx) in item.Roles"
                        :key="idx"
                        :label="val.Item1"
                        :value="val.Item2"
                        :disabled="val.Forbid"
                ...
                <el-table-column label="职位">
                  <template slot-scope="props">
                    ...
                    >
                      <el-option
                        v-for="(val,idx) in item.RoleCompetence"
                        :key="idx"
                        :label="val.Item1"
                        :value="val.Item2"
                        :disabled="val.Forbid"
              ...
              <div class="switchBox">
                <p>
                  <span>护士长</span>
                  <el-switch
                    v-model="item.IsMatron"
                    active-color="#01BF6A"
                    inactive-color="#dbdde6"
                    :active-value="true"
                    :inactive-value="false"
                    @change="((val)=>{matronSwitchChange(val,item)})"
                  ></el-switch>
                </p>
                <p>
                  <span>预设权限</span>
                  <el-switch
                    v-model="item.IsDefaultNurse"
                    active-color="#01BF6A"
                    inactive-color="#dbdde6"
                    :active-value="true"
                    :inactive-value="false"
                    @change="((val)=>{defaultSwitchChange(val,item)})"
                  ></el-switch>
                </p>
              </div>
              <el-button
                size="mini"
                type="primary"
                @click="addCompetence(index)"
                :disabled="item.AddRoleForbid"
                class="btn88x32"
              >新增权限</el-button>
            </div>
          </el-tab-pane>
        ...
        <dl>
          <dt>
            <el-select filterable default-first-option v-model="clinicSelect" class="green18x10">
              <el-option
                v-for="(item,index) in powersAndRoles.Clinics"
                :key="index"
                :label="item.ClinicName"
                :value="item.ClinicId"
              ></el-option>
...
export default {
  ...
  created() {
    ...
    axios({ url: "/api/Staff/ClinicsAndOwnerRoles" })
      .then(res => {
  ...
  methods: {
      ...
      if (
        ...
        if (
          ...
          this.editBoxData.RolesForStaff.push({
            ...
            JobAndCompetence: [],
            Roles:
              this.clinicSelect.ClinicType == 1
    ...
    editBoxSave() {
      ...
      if (
        ...
        if (
          this.GLOBAL.VerificationHandle([
            { val: val_arr1, type: "ArrayAllNotEmpty", msg: msg_arr1 }
          ])
        ) {
          let val_arr2 = [];
          let val_arr3 = [];
          ...
          for (let i = 0; i < this.editBoxData.RolesForStaff.length; i++) {
            for (
              let j = 0;
              j < this.editBoxData.RolesForStaff[i].JobAndCompetence.length;
              j++
            ) {
              val_arr2.push(
          ...
          if (
            this.GLOBAL.VerificationHandle([
              { val: val_arr2, type: "StringAllNotEmpty", msg: msg_arr2 },
              { val: val_arr3, type: "StringAllNotEmpty", msg: msg_arr3 }
            ])
          ) {
            axios({
              url: "/api/Staff",
      ...
      .el-tabs {
        ...
        .el-tabs__header {
          ...
          .el-tabs__new-tab {
            ...
            width: 50px;
            height: 30px;
            background: #00c16b url("../../assets/images/add.png") no-repeat
              center;
