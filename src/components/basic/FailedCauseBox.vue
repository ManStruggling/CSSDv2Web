    ...
    <div class="editContainer">
      <div class="editContent">
        <ul class="clear_float">
          <li>
            <p>原因名称</p>
            <el-input type="text" placeholder="失败原因名称(必填)" v-model.trim="editBoxData.Name" maxlength="10"></el-input>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    editBoxSave() {
      ...
      if(this.GLOBAL.VerificationHandle([{val:this.editBoxData.Name,type:'StringNotEmpty',msg:'名称不能为空！'}])){
        axios({ url: "/api/FailedCause", method: axiosMethod, data: this.editBoxData })
          .then(res => {
            let type;
