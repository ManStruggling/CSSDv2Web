            ...
            <div class="device_box">
                ...
                <ol class="clear_float">
                    <li v-for="(val,idx) in item.CleanPrograms" :key="idx">
                        <el-button :disabled="item.Status==0||item.DeviceType===1||(mode&&OriginDeviceId==item.Id)?false:true" :class="$route.query.deviceId==item.Id&&$route.query.programId==val.Id?'is-selected':''" @click="selectProgram(index,idx)">{{val.Name}}</el-button>
                    </li>
                </ol>
            </div>
        </li>
