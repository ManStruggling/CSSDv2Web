    ...
    <div class="container">
      <div class="content">
        ...
        <div class="selectedDate">
          <el-date-picker
            v-model="submitData.StartEndDate"
            type="daterange"
            range-separator="至"
        ...
        <el-transfer
          v-model="submitData.Filters"
          :props="{
                key: 'id',
                label: 'name'
                }"
          :data="vacationPeriods"
          :titles="['未选班表参数', '已选班表参数']"
        ></el-transfer>
      </div>
      <div class="opration">
        <p></p>
...
export default {
  ...
  methods: {
    ...
    startEndDateChange(val) {
      ...
      if (
        new Date(startDateYear, startDateMonth, 1).getMonth() !=
          new Date(endDateYear, endDateMonth, 1).getMonth() &&
        new Date(startDateYear, startDateMonth + 1, 1).getMonth() !=
          new Date(endDateYear, endDateMonth, 1).getMonth()
      ) {
      ...
      for (let i = 0; i < this.schedules.length; i++) {
        if (
          new Date(time.getTime() + 24 * 3600 * 1000)
            .toJSON()
            .substring(0, 7) == this.schedules[i].yearMonth
        ) {
          return false;
        }
      }
