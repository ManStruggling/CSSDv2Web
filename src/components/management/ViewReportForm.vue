<template>
  <div class="table_box" id="jubu">
    ...
    <el-table
      ...
      border
      height="100%"
      @filter-change="tableFilteredEvent"
    ...
    >
      <el-table-column
        ...
        :column-key="dropCol[index].SpliceName"
        sortable
  ...
  methods: {
    ...
    creatReport() {
      if (
        ...
        axios(
          `${url}&$filter=CreatedTime ge ${this.search_date[0]} and CreatedTime le ${this.search_date[1]}`
        )
          .then(res => {
        ...
        this.tableData = this.totalData.filter(data => {
          for (let key in data) {
            if (
              ...
              data[key].toLowerCase().includes(this.searchField.toLowerCase())
            ) {
              data.Visiable = true;
              return true;
            }
