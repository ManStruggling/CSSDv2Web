...
<div class="cssd_box" id="reportForm">
    ...
    <div class="cssd_table_center">
        <div class="table_box">
            <el-table :data="tableData" border row-key="Id" default-expand-all @row-click="tableRowClick">
                <div v-for="(item,index) in columnList" :key="index">
                    <el-table-column v-if="!item.isArray" :prop="item.prop" :label="item.label" sortable></el-table-column>
                    <el-table-column v-if="item.isArray" :label="item.label" :prop="item.prop" sortable type="expand" width="1">
                        <template slot-scope="props">
                            <el-table v-for="(val,idx) in item.subCols" :key="idx" border :data="props.row[val.prop]">
                                <el-table-column v-for="(subVal,subIdx) in val.arr" :key="subIdx" :prop="subVal.prop" :label="subVal.label" sortable></el-table-column>
                            </el-table>
                        </template>
                    </el-table-column>
                </div>
                    ...
                    for (let i in this.tableData[0]) {
                        if (
                            ...
                            for (let j in this.tableData[0][i][0]) {
                                if (j != "Id") {
                                    temp__arr.push({
                                        label: this.getLabel(j, "Sterilize"),
                                        prop: j
                                    });
                                }
                            }
