      ...
      <div class="title">
        <p>
          <el-select class="green24x13" v-model="selectedVacationSchedule" @change="scheduleChange">
            <el-option
              v-for="(item,index) in vacationSchedules"
              :key="index"
              :label="item.name"
              :value="item.id"
            ></el-option>
      ...
      >
        ...
        >
          <template slot-scope="props">
            <el-popover
              placement="right"
              :disabled="!props.row.Periods[index]||props.row.Periods[index].PeriodId==0?true:false"
              trigger="hover"
              :open-delay="1500"
              @show="displayPopover(props.row.Periods[index])"
            ...
            >
              ...
              <div
                slot="reference"
                class="periodItem"
              >{{matchingPeriodName(props.row.Periods[index])}}</div>
            </el-popover>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    scheduleChange(val) {
      axios({
        ...
        .then(res => {
          let vacationViewData = JSON.parse(
            decode(res.data.data.storedEvent[0].commandViewData)
          );
          ...
          if (new Date(startDate).getMonth() == new Date(endDate).getMonth()) {
            this.requestDateAccordingToSameDate(
              startDate.split("-")[0],
              startDate.split("-")[1],
              startDate.split("-")[2],
              endDate.split("-")[2]
            ...
            this.requestDataAccordingToDifferentDate(
              startDate.split("-"),
              endDate.split("-")
            );
          }
      ...
      axios({
        ...
        data: {
          query: `query getSchedule{
                            schedule(locationId:${this.UserInfo.ClinicId},yearMonth:"${year}-${month}"){
                                id
                            }
                        }`
        }
        ...
        .then(res => {
          ...
          axios({
            ...
            this.submitData.Remark = this.vacationSchedules.filter(
              element => element.id == this.selectedVacationSchedule
            )[0].remark;
          });
        })
        .catch(() => {});
    ...
    //按不同月份日期请求数据
    requestDataAccordingToDifferentDate(
      ...
      this.$http
        .all([
          axios({
            ...
            data: {
              query: `query getSchedule{
                                schedule(locationId:${this.UserInfo.ClinicId},yearMonth:"${startYear}-${startMonth}"){
                                    id
                                }
                            }`
            }
          ...
          axios({
            ...
            data: {
              query: `query getSchedule{
                                schedule(locationId:${this.UserInfo.ClinicId},yearMonth:"${endYear}-${endMonth}"){
                                    id
                                }
                            }`
            }
        ...
        .then(
          this.$http.spread((acct, perms) => {
            ...
            this.$http
              ...
              .then(
                this.$http.spread((acct2, perms2) => {
                  let startDateViewData = JSON.parse(
                    decode(acct2.data.data.storedEvent[0].commandViewData)
                  );
                  startDateViewData.Days.splice(0, startDay - 1);
                  startDateViewData.Days.forEach(element => {
                    element.day = `${startMonth}.${element.day}`;
                  });
                  let endDateViewData = JSON.parse(
                    decode(perms2.data.data.storedEvent[0].commandViewData)
                  );
                  ...
                  for (let i = 0; i < startDateViewData.Staffs.length; i++) {
                    ...
                    for (let j = 0; j < endDateViewData.Staffs.length; j++) {
                      ...
                      if (
                        startDateViewData.Staffs[i].StaffId ===
                        endDateViewData.Staffs[j].StaffId
                      ) {
                        startDateViewData.Staffs[i].Periods.push(
                          ...endDateViewData.Staffs[j].Periods
                        );
                      }
                    }
                  }
                  that.submitData = startDateViewData;
                  that.submitData.Remark = that.vacationSchedules.filter(
                    element => element.id == that.selectedVacationSchedule
                  )[0].remark;
                })
              );
          })
          ...
          for (let j = 0; j < this.submitData.Staffs[i].Periods.length; j++) {
            if (
              this.filters.includes(
                this.submitData.Staffs[i].Periods[j].PeriodId
              )
            ) {
