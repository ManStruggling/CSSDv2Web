...
<div class="cssd_totalBar tabs_totalBar" id="cleanFailedPackageList">
        ...
        <el-tab-pane v-for="(item,index) in carrierList" :key="index" :name="index+''">
            ...
            <div class="tab_content table_collapse table_unExpand">
                ...
                <el-collapse accordion>
                    <el-collapse-item v-for="(value,collapseIndex) in item.Packages" :key="collapseIndex" :name="collapseIndex+''" :class="value.IsOuterProduct?'collapseUnExpand':''">
                        <!-- 包名称 -->
                        <div slot="title" class="collapseTh">
                            ...
                            <div class="collapseTd">
                                <!-- 外来器械包or松江南院直接填写数量 -->
                                <el-input-number v-if="GLOBAL.UserInfo.HospitalVersion=='SONGJIANGNANYUAN' || value.IsOuterProduct" v-model="value.CanNotBePackagedCount" :controls="false" :min="0" :max="value.OriginalQuantity" @change="((newValue,oldValue)=>{handelNumberChange(newValue,oldValue,value)})"></el-input-number>
                                <p v-else>{{value.CanNotBePackagedCount}}</p>
                            </div>
                        </div>
                        <!-- 以器械为审核对象 -->
                        <el-table v-show="!value.IsOuterProduct" :data="value.Instruments">
                            <el-table-column label="器械名称" prop="InstrumentName" width="240" show-overflow-tooltip></el-table-column>
    ...
    <transition name="fade" enter-active-class="animated  faster fadeIn" leave-active-class="animated  faster fadeOut">
        <UnqualifiedInstruments v-if="isShowUnqualifiedInstruments" @UnqualifiedInstruments-to-father="UnqualifiedInstruments" :data="cleanQuality" :carrierIndex="carrier_index" :packageIndex="package_index" :instrumentIndex="instrument_index" :maxNum="max_num"></UnqualifiedInstruments>
    </transition>
</div>
</template>
