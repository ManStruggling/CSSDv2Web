<template>
  ...
  <div class="cssd_box tabs_half_bar" id="packageTaskList">
    ...
    <div
      :class="{cssd_table_center:true, table_collapse:true, table_unExpand:true,displayNav:displayNav}"
    >
        ...
        <div class="cssd_talbe_left_menu">
          <div class="task_origin">
            <el-select v-model="selectOrigin" @change="deselectTask" popper-class="taskOrigin">
              <el-option label="清洗完成可配包" value="TasksOfCanBePackage">
                <span>清洗完成可配包</span>
                <b>{{countPackageTasks('TasksOfCanBePackage')}}</b>
              </el-option>
              <el-option label="清洗未完成" value="PackageTasksFromCleanUndone">
                <span>清洗未完成</span>
                <b>{{countPackageTasks('PackageTasksFromCleanUndone')}}</b>
              </el-option>
              <el-option label="灭菌失败" value="PackageTasksFromSterilizeFailed">
                <span>灭菌失败</span>
                <b>{{countPackageTasks('PackageTasksFromSterilizeFailed')}}</b>
              </el-option>
              <el-option label="辅料包新任务" value="PackageTasksFromSupportMaterialProduct">
                <span>辅料包新任务</span>
                <b>{{countPackageTasks('PackageTasksFromSupportMaterialProduct')}}</b>
              </el-option>
            ...
            >
              <div slot="label" class="tabTh">
                <el-tooltip
                  ...
                  placement="right"
                  :disabled="item.ClinicName.length<14"
                >
                  <h4 class="beyondHiding">{{item.ClinicName}}</h4>
                </el-tooltip>
                <el-tooltip
                  :content="item.SubClinicName"
                  placement="right"
                  :disabled="item.SubClinicName.length<12"
                >
              ...
              <div class="tab_content table_expand">
                <div class="content_title">
                  ...
                  <p
                    style="width:60px"
                    v-show="selectOrigin=='PackageTasksFromSupportMaterialProduct'||UserInfo.JobAndCompetence.includes('000')"
                  >操作</p>
                </div>
                <el-collapse
                  accordion
                  ...
                  >
                    <div slot="title" class="collapseTh">
                      ...
                      <div class="collapseTd" style="width:250px;">
                        ...
                        <span
                          class="expeditedTag"
                          v-show="value.LostInstrumentPackageQuantity!=0"
                        >缺失 : {{value.LostInstrumentPackageQuantity}}</span>
                      </div>
                      <!-- 回收时间 -->
                      ...
                      <div class="collapseTd beyondHiding" style="width:140px;">
                        <div v-if="value.IsSingleCarrierProduct" style="width:100%;">
                          <el-tooltip
                            class="beyondHiding"
                            :content="value.SingleCarrierName"
                            placement="right"
                            :disabled="value.SingleCarrierName?value.SingleCarrierName.length<8:false"
                            style="width:100%;"
                          >
                            <span
                              class="beyondHiding"
                              style="display:block;font-weight:bold;"
                            >{{value.SingleCarrierName}}</span>
                          </el-tooltip>
                        </div>
                        <div v-else>-</div>
                      </div>
  ...
  methods: {
          ...
          for (let i = 0; i < this.tableData[key].length; i++) {
            for (
              ...
              if (
                ...
                this.currentlySelectedTask = this.tableData[key][
                  i
                ].PackageTasks[j];
            ...
            .then(res => {
              if (res.data.Code == 200) {
                this.$route.query.origin =
                  "PackageTasksFromSupportMaterialProduct";
    ...
    taskListBox2father(data, origin, tabIndex) {
      if (data) {
        if (this.connection) {
          this.connection
            .invoke("TaskUpdateNotification", data)
            .catch(function(err) {
              return console.error(err);
            });
        }
