    ...
    <div class="print_box table_unExpand">
        ...
        <div class="container">
            <ul>
                ...
                <li>
                    <p>包名称</p>
                    <el-tooltip :content="packageMsg.ProductName" placement="right" :disabled="packageMsg.ProductName?packageMsg.ProductName.length<9:true">
                        <span class="beyondHiding">{{packageMsg.ProductName}}</span>
                    </el-tooltip>
                </li>
                <li>
                    <p>外包装</p>
                ...
                <li>
                    <p>所属科室</p>
                    <el-tooltip :content="packageMsg.SubClinicName" placement="right" :disabled="packageMsg.SubClinicName?packageMsg.SubClinicName.length<9:true">
                        <span class="beyondHiding">{{packageMsg.SubClinicName}}</span>
                    </el-tooltip>
                </li>
                <li>
                    <p>包状态</p>
    ...
    methods: {
        ...
        confirmPrint() {
            if (this.packageMsg.BarCode) {
                axios({
                    url: `/api/Package/RePrintInstrumentList`,
                    method: "POST",
                    data: this.packageMsg
                }).then(res => {
                    let type;
        ...
        searchBarCode() {
            if (
                ...
                axios({
                        url: `/api/Package/RePrintInstrumentList/${this.search_str}`
                    })
                    .then(res => {
                        if (res.data.Code == 200) {
