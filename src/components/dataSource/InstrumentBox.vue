    ...
    <div class="editContainer">
        <div class="editContent">
            <ul class="clear_float">
                ...
                <li>
                    <p>安全库存</p>
                    <el-input-number type="text" placeholder="安全库存" v-model="editBoxData.SafeInventory" :min="1" :max="99999999" :controls="false" @change="((newValue,oldValue)=>{numberChange(newValue,oldValue,'SafeInventory')})"></el-input-number>
                </li>
            ...
            <transition name="fade" enter-active-class="animated fadeIn faster" leave-active-class="animated fadeOut faster">
                <!-- 拍照 -->
                <PhotoView v-if="isShowPhoto" @viewPhoto-to-father="viewPhotoToFather" :data="editBoxData.InstrumentPictures" :DeletedPicturesId="editBoxData.DeletedPicturesId"></PhotoView>
            </transition>
        </div>
...
export default {
    ...
    created() {
        ...
        axios({
                url: `/api/Instrument/InstrumentDetail/${this.editBoxData.Id}`
            })
            .then(res => {
                if (res.data.Code == 200) {
