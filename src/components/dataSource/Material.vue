<template>
  ...
  <div class="basic_main">
    <div class="basic_main_head">
      ...
      <p>
        <a
          href="\ImportBaseData\包原料汇入.xlsx"
          download="包原料汇入"
    ...
    <div class="basic_table table_unExpand">
      <el-table :data="table_data">
        ...
        <el-table-column label="规格" prop="Specification" width="210">
          <template slot-scope="props">{{props.row.Specification==""?"-":props.row.Specification}}</template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    requestData(arr, index) {
      axios({
        url: "/api/ProductMaterial/BatchImport",
        method: "POST",
        data: arr[index]
      })
        .then(res => {
    ...
    deleteThisTr(index) {
      this.showInformation({
        ...
        confirmCallBack: () => {
          axios
            .delete(`/api/ProductMaterial/${this.table_data[index].Id}`, {
              headers: {}
            })
            .then(res => {
              let type;
