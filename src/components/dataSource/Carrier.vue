  ...
  <div class="basic_main">
    ...
    >
      <PrintBarCodeList
        ...
        :data="table_data"
        :labels="[{prop:'BarCode',label:'网篮条码'},{prop:'Name',label:'网篮名称'}]"
        :printUrl="'/api/Print/Carrier'"
      ></PrintBarCodeList>
    </transition>
    <!-- 网篮编辑框 -->
    <transition
    ...
    deleteThisTr(index) {
      this.showInformation({
        ...
        confirmCallBack: () => {
          axios
            .delete(
              `/api/BasicInformation/Carrier/${this.Type}/${this.table_data[index].Id}`,
              {
                headers: {}
              }
            )
