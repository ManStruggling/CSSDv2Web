  ...
  <div class="basic_main">
    ...
    <div class="basic_table table_unExpand">
      <el-table :data="table_data">
        ...
        <el-table-column
          :label="Type==81?'发放科室':'所属科室'"
          prop="ProvideClinicName"
          width="300"
          show-overflow-tooltip
        >
          <template
            slot-scope="props"
          >{{props.row.ProvideClinicName+'/'+props.row.ProvideSubClinicName}}</template>
        </el-table-column>
  ...
  methods: {
    ...
    searchThisTableData() {
      this.table_data = this.total_data.filter(element => {
        if (
          element.Name.includes(this.basic_search.toUpperCase()) ||
          (element.ShortCode
            ? element.ShortCode.includes(this.basic_search.toUpperCase())
            : false)
        ) {
          return true;
        }
