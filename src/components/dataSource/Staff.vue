  ...
  <div class="basic_main">
    ...
    >
      <PrintBarCodeList
        ...
        :data="table_data"
        :labels="[{prop:'BarCode',label:'人员条码'},{prop:'Name',label:'人员名称'}]"
        :printUrl="'/api/Print/Staff'"
      ></PrintBarCodeList>
    </transition>
    <!-- 人员编辑框 -->
    <transition
  ...
  methods: {
    ...
    fileChange(evt) {
      ...
      reader.onload = event => {
        ...
        for (let i = 0; i < objData.Sheet1.length; i++) {
          objData.Sheet1[i].RolesForStaff = objData.RolesForStaff.filter(
            val => {
    ...
    deleteThisTr(index) {
      this.showInformation({
        ...
        confirmCallBack: () => {
          axios
            .delete(`/api/Staff/${this.table_data[index].Id}`)
            .then(res => {
              let type;
              if (res.data.Code == 200) {
                type = "success";
