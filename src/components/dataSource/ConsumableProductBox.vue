      ...
      <div class="editContent">
        <ul class="clear_float">
          ...
          <li>
            <p>最小规格数</p>
            <el-input-number
              v-model.trim="editBoxData.MinimumUnit"
              :min="1"
              :max="999999"
              :controls="false"
              placeholder="最小规格数"
              @change="((newValue,oldValue)=>{numberChange(newValue,oldValue,'MinimumUnit')})"
            ></el-input-number>
          </li>
          <li>
            <p>单价</p>
            <el-input-number
              placeholder="单价"
              v-model="editBoxData.Price"
              :min="1"
              :max="999999"
              :controls="false"
              @change="((newValue,oldValue)=>{numberChange(newValue,oldValue,'Price')})"
            ></el-input-number>
          </li>
          <li>
            <p>请领代码</p>
          ...
          <li>
            <p>请领部门</p>
            <el-select
              v-model="editBoxData.ReceiveDepartmentId"
              placeholder="请领部门(必填)"
              class="green24x13"
            >
              <el-option
                v-for="(item,index) in receiveDepartments"
                :key="index"
                :label="item.name"
                :value="item.id"
              ></el-option>
