    ...
    <div class="cssd_title">
      ...
      <div class="cssd_title_right">
        <p>
          <span>借包科室</span>
          <el-select
            v-model="submitData.BorrowedSubClinicId"
            filterable
...
export default {
  ...
  created() {
    ...
    axios({ url: `/api/Clinic/SubClinic` })
      .then(res => {
  ...
  methods: {
    ...
    submitComplete(){
      if (
        ...
        axios({url:`/api/Provide/BorrowedPackageComplete`,method:"POST",data:this.submitData}).then(res=>{
          let type;
          if(res.data.Code==200){
            type="success";
            this.$router.go(0);
    ...
    handleBarCode(msg){
      ...
      if(onOff){
        axios({url:`/api/Scanner/IncludePackages/Provide/${msg}`}).then(res=>{
          if(res.data.Code==200){
