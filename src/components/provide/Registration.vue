        ...
        <div class="cssd_talbe_left_menu">
          <el-tabs :tab-position="'left'" v-model="tabActiveName" @tab-click="handleTabClick">
            <el-tab-pane v-for="(item,index) in provideTaskList" :key="index" :name="index+''">
              <div
                slot="label"
                v-show="showAllTask?true:(UserInfo.HospitalVersion!='YANCHENGFUBAO'?true:item.RemainInventoryTotalQuantity)"
                class="el-tabs__item_clinic"
              >
                <el-tooltip
                  :content="item.ClinicName"
                  placement="right"
                  :disabled="item.ClinicName.length<12"
                >
                  <h3 class="beyondHiding">{{item.ClinicName}}</h3>
                </el-tooltip>
                <p>
                  <span v-if="UserInfo.HospitalVersion=='YANCHENGFUBAO'">
                    <span>剩余总库存数:</span>
              ...
              <div
                class="tab_content table_collapse table_unExpand"
                v-show="showAllTask?true:(UserInfo.HospitalVersion!='YANCHENGFUBAO'?true:item.RemainInventoryTotalQuantity)"
              >
                <div class="selectSubClinic">
                  ...
                  <el-select
                    ...
                    v-model="item.SelectedSubClinicId"
                    class="green24x13"
                    @change="((val)=>{provideSubClinicChange(val,index)})"
                    :filterable="true"
                  >
                    ...
                    >
                      <el-tooltip
                        ...
                        placement="right"
                        :disabled="value.SubClinicName.length<9"
                      >
                ...
                <div class="content_title">
                  ...
                  <p>回收时间</p>
                  <p>发放科室</p>
                  <p style="width:90px;" v-if="UserInfo.HospitalVersion!='YANCHENGFUBAO'">预计发放数</p>
                  <p style="width:80px;">库存数</p>
                  <p style="width:100px;" v-if="UserInfo.HospitalVersion!='YANCHENGFUBAO'">剩余发放数</p>
                ...
                <el-collapse
                  v-model="activeName"
                  accordion
                  @change="((index)=>{handleCollapseChange(index,item.SubClinicTasks[item.SelectedSubClinicId].ProvideTaskDetails)})"
                >
                  <el-collapse-item
                  ...
                  >
                    <div slot="title" class="collapseTh">
                      ...
                      <!-- 发放科室 -->
                      <div class="collapseTd">
                        <p v-if="UserInfo.HospitalVersion=='YANCHENGFUBAO'&&value.IsCommonProduct">-</p>
                        <p v-else>
                          <el-tooltip
                            :content="value.ProvideSubClinic"
                            placement="right"
                            :disabled="value.ProvideSubClinic.length<11"
                          >
                      ...
                      <div
                        class="collapseTd"
                        style="width:130px;"
                        v-if="UserInfo.HospitalVersion!='YANCHENGFUBAO'"
                      >
                      ...
                      <div class="collapseTd" style="width:140px;">
                        <!-- 条码包 -->
                        <p
                          v-if="!value.IsNotPrintBarCode&&!value.IsDisposableProduct&&!value.IsNumberProduct"
                        >{{countThisProvideNumber(value)}}</p>
                        <!-- 一次性物品 -->
                        <p
                          v-if="value.IsDisposableProduct"
                        >{{countDisposableThisProvideNumber(value)}}</p>
                        <!-- 计数包 -->
                        ...
                        >{{countNumberProductThisProvideNumber(value)}}</p>
                      </div>
                      <!-- 操作 -->
                      <div class="collapseTd" style="width:90px;">
                        <a class="edit_a" @click.stop="editScheduledQuantity(value)">编辑</a>
                        <a
                          v-if="value.ProvideTaskGenerateType==3||UserInfo.JobAndCompetence.includes('000')"
                          @click.stop="deleteThisTask(value.ProvideTaskId)"
                        >删除</a>
                        <p v-else>-</p>
                      </div>
                    </div>
                    <el-table
                      :data="value.ProvidePackages"
                      v-if="!value.IsNotPrintBarCode&&!value.IsDisposableProduct"
                    >
                      ...
                      <el-table-column label="操作" width="210">
                        <template slot-scope="props">
                          <el-button
                            @click="deleteProvideProduct(value.ProvidePackages,props.$index)"
                          >删除</el-button>
                        </template>
                      </el-table-column>
                    ...
                    >
                      <el-table-column label="批号" width="240">
                        <template slot-scope="props">
                          <el-select
                            ...
                            class="green18x10"
                            @change="((val)=>{handleBatchNumberChange(val,value,props.$index)})"
                          >
                            <el-option
                              v-for="(batchItem,batchIndex) in value.DisposableBatchNumbers"
                              :key="batchIndex"
                              :label="batchItem.BatchNumber"
                              :value="batchItem.BatchNumber"
                            ></el-option>
                          </el-select>
                        </template>
                      </el-table-column>
                      ...
                      <el-table-column label="数量" prop="Quantity" width="210">
                        <template slot-scope="props">
                          <el-input-number
                            v-model.trim="props.row.Quantity"
                            :controls="false"
                            :min="0"
                            :max="props.row.InventoryQuantity"
                            @change="((newValue,oldValue)=>{handleDisposableNumberChange(newValue,oldValue,value,props.$index,item.SubClinicTasks[item.SelectedSubClinicId].ProvideTaskDetails)})"
                            :disabled="!props.row.BatchNumber"
                          ></el-input-number>
                        </template>
                      </el-table-column>
                      <el-table-column label="操作" width="210">
                        <template slot-scope="props">
                          <el-button
                            @click="deleteProvideProduct(value.DisposableProducts,props.$index)"
                          >删除</el-button>
                        </template>
                      </el-table-column>
                      <el-table-column></el-table-column>
                    ...
                    <el-button
                      v-if="!value.IsNotPrintBarCode&&value.IsDisposableProduct"
                      class="btn88x32 insertDisposableProduct"
                      @click="insertDisposableProduct(value.DisposableProducts)"
                    >新增批次</el-button>
                  </el-collapse-item>
                </el-collapse>
    ...
    >
      <ol>
        ...
        <li>
          <p>现预计发放数</p>
          <el-input-number
            v-model="adjustedprovideTaskMsg.ScheduledQuantity"
            :controls="false"
            :min="0"
            :max="999"
            @change="handleAdjustedScheduledNumberChange"
          ></el-input-number>
        </li>
      </ol>
...
export default {
  ...
  created() {
    ...
    axios("/api/Provide/ProvideTasks")
      .then(res => {
        if (res.data.Code == 200) {
          ...
          if (this.UserInfo.HospitalVersion == "YANCHENGFUBAO") {
            ...
            for (let i = 0; i < res.data.Data.length; i++) {
              for (let j = 0; j < res.data.Data[i].ProvideTasks.length; j++) {
                if (res.data.Data[i].ProvideTasks[j].IsCommonProduct) {
                  let commonProductTask = res.data.Data[i].ProvideTasks.splice(
                    j,
                    1
                  )[0]; //该次通用包任务
                  j -= 1;
                  ...
                  if (checkRepeat == "") {
                    ...
                    //存在
                    checkRepeat[0].RemainQuantity +=
                      commonProductTask.RemainQuantity;
                  }
                }
              }
  ...
  methods: {
    ...
    provideSubClinicChange(val, index) {
      this.provideTaskList[index].ProvideTasks.forEach(element => {
    ...
    taskSortByRecycleTime(val, index) {
      this.provideTaskList[index].SubClinicTasks[val].ProvideTaskDetails.sort(
        (a, b) => {
    ...
    getSubClinicTasks(index) {
      for (let j = 0; j < this.provideTaskList[index].SubClinics.length; j++) {
        //循环子科室
        this.provideTaskList[index].SubClinicTasks[
          this.provideTaskList[index].SubClinics[j].SubClinicId
        ] = {
          ProvideSubClinicId: this.provideTaskList[index].SubClinics[j]
            .SubClinicId,
          ThisClinicProvideNumber: 0,
          ProvideTaskDetails: []
        };
        if (this.UserInfo.HospitalVersion == "YANCHENGFUBAO" && index == "0") {
          this.provideTaskList[index].SubClinicTasks[
            this.provideTaskList[index].SubClinics[j].SubClinicId
          ...
          for (
            let k = 0;
            k < this.provideTaskList[index].ProvideTasks.length;
            k++
          ) {
            if (
              this.provideTaskList[index].SubClinics[j].SubClinicId ===
              this.provideTaskList[index].ProvideTasks[k].ProvideSubClinicId
            ) {
              this.provideTaskList[index].SubClinicTasks[
                this.provideTaskList[index].SubClinics[j].SubClinicId
              ].ProvideTaskDetails.push(
                this.provideTaskList[index].ProvideTasks[k]
              );
            }
          }
        }
    ...
    handleBatchNumberChange(val, value, index) {
      ...
      for (let i = 0; i < value.DisposableProducts.length; i++) {
        ...
        if (
          value.DisposableProducts[i].BatchNumber &&
      ...
      value.DisposableBatchNumbers.forEach(item => {
        if (item.BatchNumber == val) {
          ...
          value.DisposableProducts[index].InventoryQuantity =
    ...
    handleCollapseChange(index, tasks) {
      if (index) {
        ...
        if (
          disposableTaskItem.IsDisposableProduct &&
          !disposableTaskItem.Requested
        ) {
          axios({
        ...
        for (let i = 0; i < tasks.length; i++) {
          if (tasks[i].ProductId == value.ProductId) {
            for (let j = 0; j < tasks[i].DisposableProducts.length; j++) {
              if (
                tasks[i].DisposableProducts[j].BatchNumber ==
                value.DisposableProducts[index].BatchNumber
              ) {
      ...
      if (this.provideTaskList.length > 0) {
        ...
        for (let i = 0; i < currentTaskList.length; i++) {
          if (currentTaskList[i].ProvidePackages) {
            for (
              let j = 0;
              j < currentTaskList[i].ProvidePackages.length;
              j++
            ) {
      ...
      if (
        ...
        for (let i = 0; i < tasks.length; i++) {
          if (tasks[i].IsDisposableProduct) {
            for (let j = 0; j < tasks[i].DisposableProducts.length; j++) {
              numberCanNotBeZeroOfVerify.push(
                tasks[i].DisposableProducts[j].Quantity
              );
    ...
    handleAddData(data) {
      ...
      for (let j = 0; j < currentTaskList.length; j++) {
        ...
        if (
          currentTaskList[j].ExpeditedPackageQuantity &&
      ...
      return (obj, index) => {
        ...
        for (let i = 0; i < obj.ProvideTaskDetails.length; i++) {
          if (obj.ProvideTaskDetails[i].IsDisposableProduct) {
            for (
              let j = 0;
              j < obj.ProvideTaskDetails[i].DisposableProducts.length;
              j++
            ) {
