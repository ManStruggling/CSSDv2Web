      ...
      <el-collapse accordion @change="collapseChange">
        ...
        >
          <div slot="title" class="collapseTh">
            ...
            <div class="collapseTd">
              <el-tooltip
                :content="item.SubClinic"
                placement="right"
                :disabled="item.SubClinic.length<12"
              >
            ...
            <div class="collapseTd">
              <p>
                <el-button @click.stop="handlePrint(item.Id)">打印清单</el-button>
              </p>
            </div>
          </div>
          <el-tabs type="card">
            <el-tab-pane label="无菌物品" v-if="item.Packages==''||item.Packages===null?false:true">
              <el-table :data="item.Packages" :default-expand-all="true">
                ...
                <el-table-column label="包内个数" prop="NumberProductQuantity" width="210">
                  <template
                    slot-scope="props"
                  >{{props.row.NumberProductQuantity?props.row.NumberProductQuantity:'-'}}</template>
                </el-table-column>
            ...
            <el-tab-pane
              label="一次性物品"
              v-if="item.DisposableProducts==''||item.DisposableProducts===null?false:true"
            >
  ...
  methods: {
    ...
    collapseChange(index) {
      if (
        ...
        axios({
          url: `/api/Provide/PackagesInProvideLogisticsId/${this.recordList[index].Id}`
        }).then(res => {
          if (res.data.Code == 200) {
            this.recordList[index].Packages = res.data.Data.Packages;
            this.recordList[index].DisposableProducts =
              res.data.Data.DisposableProducts;
          } else {
    ...
    searchRecordsData() {
      this.GLOBAL.searchRecord(
        `/api/Provide/ProvideRecords/${this.search_date[0]}/${this.search_date[1]}`,
        this
      );
    },
    //打印
    handlePrint(Id) {
      axios({
        url: `/api/Provide/PrintProvideList?provideTaskId=${Id}`
      })
        .then(res => {
          if (res.data.Code == 200) {
