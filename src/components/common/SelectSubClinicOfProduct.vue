<template>
  <div class="basic_lost_package_box">
    <div class="package_box">
      <h3>
        <el-select
          v-model="selectedSubClinicId"
          class="green24x13"
          :filterable="true"
          @change="selectedSubClinicIdChange"
        >
          <el-option
          ...
          >
            <el-tooltip
              ...
              placement="right"
              :disabled="item.ProvideSubClinicName.length<8"
            >
      ...
      <el-table
        ref="multipleTable"
        :data="selectedSubClinicId?packageList:[]"
        tooltip-effect="dark"
        style="width: 100%"
        max-height="300"
        width="400"
...
export default {
  ...
  methods: {
      ...
      if (
        ...
        if (this.submitApi) {
          ...
          if (
            ...
            this.$emit(
              "selectSubClinicOfProduct-to-father",
    ...
    //判断参数范围
    judgedParameters() {
      let odataBaseRequestUrl = this.getApiLimit
        ? `/odata/AllProducts?$filter=(${this.getApiLimit}) and `
        : "/odata/AllProducts?$filter=";
      if (this.submitApi) {
        this.getUrl =
          this.packageBoxClassSelect == "all"
            ? `${odataBaseRequestUrl}((${this.requestApi}) or (ProvideSubClinicId eq ${this.selectedSubClinicId} and ProvideGenerateType eq '手动生成'))`
            : `${odataBaseRequestUrl}((${
                this.requestApi
              }) or (ProvideSubClinicId eq ${
                this.selectedSubClinicId
              } and ProvideGenerateType eq '手动生成')) and (type eq ${encodeURI(
                "'" + this.packageBoxClassSelect + "'"
              )})`;
      } else {
        this.getUrl =
          this.packageBoxClassSelect == "all"
            ? `${odataBaseRequestUrl}(${this.requestApi}) and (ProvideSubClinicId eq ${this.selectedSubClinicId} or IsCommonProduct)`
            : `${odataBaseRequestUrl}(${
                this.requestApi
            ...
            if (res.data.value[i].IsCommonProduct) {
              ...
              for (let j = 0; j < this.clinicList.length; j++) {
                if (
                  ...
                  res.data.value[i].ProvideClinicId = this.clinicList[
                    j
                  ].ProvideClinicId;
                  break;
                }
