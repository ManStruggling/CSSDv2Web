  ...
  methods: {
      ...
      if(val.SingleCarrierName){
        axios({url:`/api/Scanner/NewPurchasing/SingleProductCarrier/${val.SingleCarrierName}`}).then(res=>{
          if(res.data.Code==200){
            ...
            this.singleCarrierIds.push(res.data.Data.SingleCarrierId);
            val.Forbid=true;
          }else{
            this.showInformation({classify:"message",msg:res.data.Msg});
          }
    ...
    submitComplete() {
      ...
      for (let i = 0; i < this.newPurchasingData.length; i++) {
        for (let j = 0; j < this.newPurchasingData[i].Packages.length; j++) {
          this.submitData.Products.push(this.newPurchasingData[i].Packages[j]);
        }
      }
      ...
      let singleCarrierIds=[];
      for(let i=0;i<this.newPurchasingData.length;i++){
        for(let j=0;j<this.newPurchasingData[i].Packages.length;j++){
          if(this.newPurchasingData[i].Packages[j].IsSingleCarrierProduct){
            singleCarrierIds.push(this.newPurchasingData[i].Packages[j].SingleCarrierId);
          }
        }
      }
      if (
        this.GLOBAL.VerificationHandle([
          ...
          },
          {
            ...
            val:singleCarrierIds,
            type:"NumberNotAllZero",
            msg:"单网篮包必须绑定网篮！"
          }
        ])
      ) {
        if(singleCarrierIds.length===Array.from(new Set(singleCarrierIds)).length){
          ...
          this.showInformation({classify:"message",msg:"扫入的单包网篮不能重复！",})
        }
      }
    },
    //处理手工录入
    ...
    packageNumberChange(newValue, oldValue, index, idx) {
      if (newValue == 0||newValue==undefined) {
        this.$confirm("您确定要删除该包?", "提示", {
          ...
          .then(() => {
            this.singleCarrierIds = this.singleCarrierIds.filter(value=>{return value!=this.newPurchasingData[index].Packages[idx].SingleCarrierId});
      ...
      for (let i = 0; i < this.newPurchasingData.length; i++) {
        if (
          ...
          if(data.IsSingleCarrierProduct){
            ...
            // 一般包
            for (let j = 0; j < this.newPurchasingData[i].Packages.length; j++) {
              if (this.newPurchasingData[i].Packages[j].ProductId == data.ProductId) {
                //找到包类型
    ...
    handleBarCode(msg){
      if(/^QXW/.test(msg.toUpperCase())){
        axios({url:`/api/Scanner/Recycle/${msg}`}).then(res=>{
          if(res.data.Code==200){
