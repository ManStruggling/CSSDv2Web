    ...
    <div class="cssd_table_center tabs_totalBar cssd_totalBar">
        ...
        <div class="cssd_table_bottom">
            ...
            <p>
                ...
                <el-button type="primary" @click="submitComplete">{{isNewpurchasingChangeMode?'修改完成':'录入完成'}}</el-button>
            </p>
        </div>
    </div>
    <transition name="fade" enter-active-class="animated fadeIn faster" leave-active-class="animated fadeOut faster">
        <!-- 手工录入 -->
        <ManualEnter v-if="isShowManualEnter" @to-father="carrierData2father" :BarCodeList="[]" :ApiUrl="'/api/Scanner/Recycle'" :OnlyQXW="true"></ManualEnter>
    </transition>
    <transition name="fade" enter-active-class="animated fadeIn faster" leave-active-class="animated fadeOut faster">
        <!-- 数量包登记   控制是否挂载   数据通信   类型 -->
        <PackageList v-if="isShowPackageList" @packageList-to-father="packgeList2father" :requestApi="`type eq '高水平消毒包' or type eq '追溯的无菌包'`" :packageClass="'NewPurchasing'"></PackageList>
    </transition>
</div>
</template>
            ...
            for (let i = 0; i < this.newPurchasingData.length; i++) {
                if (
                    ...
                    if (data.IsSingleCarrierProduct) {
                        ...
                        // 一般包
                        for (
                            let j = 0; j < this.newPurchasingData[i].Packages.length; j++
                        ) {
                            if (
