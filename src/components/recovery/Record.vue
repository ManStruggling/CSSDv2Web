        ...
        <el-collapse accordion @change="collapseChange">
            <el-collapse-item v-for="(item,collapseIndex) in recordList" :key="collapseIndex" :name="collapseIndex+''">
                <div slot="title" class="collapseTh">
                    ...
                    <div class="collapseTd">
                        <p>
                            <el-button size="mini" @click.stop="changeRecycleRecord(collapseIndex)" :disabled="(GLOBAL.UserInfo.JobAndCompetence.includes('111')||GLOBAL.UserInfo.JobAndCompetence.includes('000')||GLOBAL.UserInfo.JobAndCompetence.includes('100'))&&!item.CanNotBeModified?false:true">修改</el-button>
                        </p>
                    </div>
                </div>
...
export default {
    ...
    created() {
        this.mode = this.$route.query.sourceRoute == '/recovery/newPurchasing' ? 2 : 1;
    ...
    methods: {
        ...
        searchRecordsData() {
            this.GLOBAL.searchRecord(`${this.$route.query.requestUrl}/${this.search_date[0]}/${this.search_date[1]}`, this);
        },
