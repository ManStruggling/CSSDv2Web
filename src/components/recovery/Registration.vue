<template>
  ...
  <div class="cssd_box" id="recoveryRegistration">
        ...
        <p class="routeOfStainedCar">
          ...
          <el-select v-model="recoveryData.RecycleRouteId">
            <el-option
              ...
              :key="index"
              :label="item.RouteBarCode"
              :value="item.RecycleRouteId"
            >
    ...
    <div
      :class="{cssd_table_center:true, cssd_table_expand:true, tabs_totalBar:true, cssd_totalBar:true, displayNav:displayNav}"
    >
        ...
        <el-tab-pane v-for="(item,index) in recoveryData.Clinics" :key="index" :name="index +''">
          ...
          <div class="tab_content table_collapse">
            ...
            <el-collapse v-model="collapseActiveName" accordion>
              <el-collapse-item
                v-for="(value,collapseIndex) in item.RecycleProducts"
                :key="collapseIndex"
                :name="collapseIndex+''"
                :class="value.IsNotPrintBarCode||value.IsLostPackage?'collapseUnExpand':''"
              >
                ...
                <div slot="title" class="collapseTh">
                  ...
                  <div class="collapseTd" style="width:140px;">
                    ...
                    <p v-else>
                      <el-input-number
                        ...
                        :min="0"
                        :max="value.ProductQuantity>value.NumberOfBorrowedProduct?value.NumberOfBorrowedProduct:value.ProductQuantity"
                        :controls="false"
                        @click.stop.native="GLOBAL.cancelBubble"
                      ></el-input-number>
                    </p>
                  </div>
                  <!-- 加急包 ##所有包都可加急-->
                  <div class="collapseTd" style="width:100px;">
                    <p>
                      <el-input-number
                        v-model="value.NumberOfExpeditedProduct"
                        :min="0"
                        :max="value.ProductQuantity-value.NumberOfReturnProduct"
                        :controls="false"
                        @change="((newValue)=>{expeditedNumberChange(newValue,item.RecycleProducts,collapseIndex)})"
                        @click.stop.native="GLOBAL.cancelBubble"
                      ></el-input-number>
                    </p>
                  </div>
                ...
                <div class="table_expand">
                  ...
                  >
                    ...
                    <el-table-column
                      label="应回收器械数"
                      prop="ShouldBeRecycledInstrumentTotalQuantity"
                      width="210"
                    ></el-table-column>
                    <el-table-column
                      label="实回收器械数"
                      prop="ActuallyRecycleInstrumentTotalQuantity"
                      width="210"
                    ></el-table-column>
                    <el-table-column label="操作" width="210">
                      <div slot-scope="scope">
                        <el-button
                          @click.stop="handleDeletePackage(scope.$index, scope.row,value.RecyclePackageIds,collapseIndex,index)"
                        >删除</el-button>
                      </div>
                    </el-table-column>
                    ...
                    <el-table-column type="expand" width="1">
                      <template slot-scope="props">
                        <div class="table_detail">
                          ...
                          <ul>
                            <li
                              v-for="(selfs,myTableIndex) in props.row.Instruments"
                              :key="myTableIndex"
                            >
                              <p>
                              ...
                              <p>
                                <el-input-number
                                  v-model="selfs.ActuallyRecycleQuantity"
                                  :min="0"
    ...
    >
      ...
      <PackageList
        ...
        :packageClass="'NotBarCodeProducts'"
    ...
    >
      <!-- 通用包分配科室 -->
      <AllocatedPackages
        v-if="isShowAllocatedPackages"
        @allocated-to-father="allocatedToFather"
        :list="waitToAllocation"
      ></AllocatedPackages>
    </transition>
    <transition
      name="fade"
    ...
    >
      ...
      <SelectSubClinicOfProduct
        v-if="isShowLostPackageList"
        @selectSubClinicOfProduct-to-father="lostPackageToFather"
        :requestApi="`type eq '高水平消毒包' or type eq '追溯的无菌包'`"
        :getApiLimit="`IsNotPrintBarCode eq false and IsSingleCarrierProduct eq false`"
      ></SelectSubClinicOfProduct>
    </transition>
  </div>
</template>
  ...
  methods: {
    ...
    getBarCodeArray() {
      ...
      for (let i = 0; i < this.recoveryData.Clinics.length; i++) {
        for (
          let j = 0;
          j < this.recoveryData.Clinics[i].RecycleProducts.length;
          j++
        ) {
          if (
        ...
        if (ClinicList[i].ProvideSubClinicId === data.ProvideSubClinicId) {
          ...
          for (let j = 0; j < ClinicList[i].RecycleProducts.length; j++) {
            if (
              ...
              this.recoveryData.Clinics[i].RecycleProducts[
                j
              ].NumberOfExpeditedProduct = data.NumberOfExpeditedProduct;
              return;
            }
            //计数包
            ...
            if (
              ...
              ClinicList[i].RecycleProducts[j].IsNotPrintBarCode
            ) {
              //计数包 finded 包类型
              NoPackageClass = false;
