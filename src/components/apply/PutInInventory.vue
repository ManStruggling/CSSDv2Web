      ...
      <div class="cssd_title_right">
        <p>
          ...
          <el-select v-model="putInTask.InboundSubClinicId" filterable default-first-option class="white24x13">
            <el-option 
              v-for="(item,index) in GLOBAL.UserInfo.SubClinics"
              :key="index"
              ...
              <div class="collapseTd">
                ...
                <p v-if="value.IsNotPrintBarCode">
                  <el-input-number
                    ...
                    :min="0" :max="value.RemainInboundCount"
                    :controls="false"
...
export default {
  ...
  created() {
    ...
    axios({url:`/api/Inbound`}).then(res=>{
  ...
  methods: {
    ...
    //入库完成
    submitComplete(){
      if(this.GLOBAL.VerificationHandle([{val:this.putInTask.InboundNumber,type:"NumberNotZero",msg:"您还没有添加包，请至少扫入一个包！"},{val:this.putInTask.InboundSubClinicId,type:"StringNotEmpty",msg:"请选择入库科室！"}])){
        axios({url:`/api/Inbound`,method:"POST",data:this.putInTask}).then(res=>{
          let type;
          if(res.data.Code==200){
            type="success";
            this.putInTask = res.data.Data;
    ...
    handleBarCode(msg){
      ...
      if(onOff){
        axios({url:`/api/Scanner/Inbound/${msg}`}).then(res=>{
          if(res.data.Code==200){
