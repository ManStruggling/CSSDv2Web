    ...
    <div class="cssd_title">
      ...
      <div class="cssd_title_right">
        <p>
          <span>入库科室</span>
          <el-select
            v-model="putInTask.InboundSubClinicId"
            filterable
            default-first-option
            class="white24x13"
          >
            <el-option
              v-for="(item,index) in UserInfo.SubClinics"
              :key="index"
    ...
    <div class="cssd_table_center cssd_totalBar">
      <div class="table_box table_collapse table_unExpand">
        ...
        <el-collapse accordion v-model="activeName">
          <el-collapse-item
            v-for="(value,collapseIndex) in putInTask.Products"
            :key="collapseIndex"
            :name="collapseIndex+''"
            ...
            <div slot="title" class="collapseTh">
              ...
              <div class="collapseTd">
                ...
                <p v-if="value.IsNotPrintBarCode">
                  <el-input-number
                    v-model="value.ThisTimeInboundCount"
                    :min="0"
                    :max="value.RemainInboundCount"
                    :controls="false"
                    @click.native.stop="GLOBAL.cancelBubble"
                    @change="((newValue,oldValue)=>{handleNumberChange(newValue,oldValue,collapseIndex)})"
                  ></el-input-number>
                </p>
              </div>
            </div>
        ...
        for (let i = 0; i < data.length; i++) {
          for (let j = 0; j < this.putInTask.Products.length; j++) {
            if (data.IsNumberProduct) {
              if (
                data[i].ProductId === this.putInTask.Products[j].ProductId &&
                this.putInTask.Products[j].ThisTimeInboundCount +
                  data[i].NumberProductQuantity <=
                  this.putInTask.Products[j].RemainInboundCount
              ) {
    ...
    countPackagesNumber() {
      return index => {
        if (this.putInTask.Products[index].IsNumberProduct) {
          ...
          this.putInTask.Products[index].ThisTimeInboundCount =
            this.putInTask.Products[index].Packages == undefined
              ? 0
              : this.putInTask.Products[index].Packages.length;
