  ...
  <div class="cssd_box" id="clinicApply">
      ...
      <div class="table_box">
        ...
        <el-table :data="submitData.Packages.concat(submitData.OlderSystemPackages)">
          ...
          <el-table-column label="原住院号" width="210">
            <template
              slot-scope="props"
            >{{props.row.PatientHospitalId?props.row.PatientHospitalId:"-"}}</template>
          </el-table-column>
    ...
    >
      ...
      <ManualEnter
        ...
        :BarCodeList="submitData.Packages.concat(submitData.OlderSystemPackages)"
  ...
  methods: {
    ...
    applySubmit() {
      ...
      if (
        this.GLOBAL.VerificationHandle([
          ...
          {
            val: this.submitData.Packages.concat(
              this.submitData.OlderSystemPackages
            ),
            type: "ArrayNotEmpty",
        ...
        if (this.isHasPakcageOfDifferentHospitalId) {
          this.showInformation({
            classify: "confirm",
            msg:
              "外来器械包绑定的住院号与本次使用的病人住院号不同，确定要使用包吗？",
            confirmCallBack: () => {
              if (this.UserInfo.HospitalVersion == "TONGJI") {
                if (
                  this.UserInfo.JobAndCompetence.includes("000") ||
                  this.UserInfo.JobAndCompetence.includes("200") ||
                  this.UserInfo.JobAndCompetence.includes("271")
                ) {
    ...
    handleBarCode(msg) {
      if (this.submitData.Patient.HospitalId) {
        let onOff = true;
        this.submitData.Packages.concat(
          this.submitData.OlderSystemPackages
        ).forEach(item => {
          //发现已录入
