  ...
  <div class="cssd_box" id="clinicApply">
    ...
    <div class="cssd_table_center cssd_totalBar table_unExpand">
        ...
        <ul v-if="GLOBAL.UserInfo.HospitalVersion=='TONGJI'">
          <li>
            ...
            <div class="el_input_box font16blod">
              <el-input
                v-model="submitData.Patient.HospitalId"
                :disabled="true"
              ></el-input>
            </div>
          </li>
      ...
      <div class="cssd_table_bottom">
        <p>
          共计
          <span>{{countPackageNumber(submitData.Packages)}}</span> 包
        </p>
        <p>
...
export default {
  ...
  methods: {
    ...
    //取消修改
    cancelChange() {
      this.$router.push({ path: "/apply/record" });
    },
    //确认使用
    applySubmit() {
    ...
    handleShowManualEnter() {
      if (this.isChangeMode || this.submitData.Patient.HospitalId) {
        ...
        this.showInformation({classify:"message",msg:"您还没有查询住院号！请查询住院号再录入包！"});
      }
    },
    //手工录入数据传递
            ...
            this.$confirm(
              ...
              .then(() => {
                ...
                this.$alert(msg, "警告", {
                  confirmButtonText: "确定",
                  callback: action => {
                    this.showInformation({classify:"message",msg:"您使用的外来器械包和该病人的住院号不同，请重打包条码！",type:"warning"});
                  }
                });
              })
              .catch(() => {
    ...
    handleBarCode(msg){
      if(this.submitData.Patient.HospitalId){
        ...
        this.showInformation({classify:"message",msg:"您还没有查询住院号，请查询住院号再扫描包条码！",type: "warning"});
      }
    }
  },
  computed: {
