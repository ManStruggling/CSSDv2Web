...
export default {
  ...
  props:["cssdId"],
  created() {
    axios({url:`/odata/AllProducts?$filter=type eq '一次性物品'`,headers:{CssdId:this.$props.cssdId}}).then(res=>{
  ...
  methods: {
    ...
    packageSearch() {
      //code
      let url;
      url = `/odata/AllProducts?$filter=type eq '一次性物品' and (contains(ProductShortCode,${"'" +this.searchShortCode +"'"}) or contains(ProductName,${"'" +this.searchShortCode +"'"}))`
