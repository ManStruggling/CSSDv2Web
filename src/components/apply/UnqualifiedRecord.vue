            ...
            <div class="collapseTd">
              <p>
                <el-button
                  ...
                  :disabled="(UserInfo.JobAndCompetence.includes('271')||UserInfo.JobAndCompetence.includes('000')||UserInfo.JobAndCompetence.includes('200'))&&!item.CanNotBeModified?false:true"
                >修改</el-button>
              </p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleShowPhotos(index, idx) {
      axios({
        ...
        if (res.data.Code == 200) {
          this.recordList[index].FailedPackages[idx].FailedPackagePictures =
            res.data.Data;
          this.photoData = this.recordList[index].FailedPackages[
            idx
    ...
    collapseChange(index) {
      if (
        index != "" &&
        (this.recordList[index].FailedPackages == "" ||
          this.recordList[index].FailedPackages === null)
      ) {
        axios({
    ...
    searchRecordsData() {
      this.GLOBAL.searchRecord(
        `/api/FailedPackage/FailedPackageRecords/${this.search_date[0]}/${this.search_date[1]}`,
        this
      );
    }
  },
