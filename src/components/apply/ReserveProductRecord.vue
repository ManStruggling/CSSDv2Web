            ...
            <div class="collapseTd">
              <p>
                <el-button
                  ...
                  :disabled="(GLOBAL.UserInfo.JobAndCompetence.includes('271')||GLOBAL.UserInfo.JobAndCompetence.includes('000')||GLOBAL.UserInfo.JobAndCompetence.includes('200'))&&!item.CanNotBeModified?false:true"
                >修改</el-button>
                <el-button
                  size="mini"
...
export default {
  ...
  methods: {
    ...
    //取消预订
    cancelReserve(index){
      axios({url:`/api/Book/${this.recordList[index].Id}`,method:"DELETE"}).then(res=>{
        let type;
        if(res.data.Code==200){
          type="success";
          //socket发送信息
