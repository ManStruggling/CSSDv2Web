  ...
  <div class="cssd_box" id="hasBarCodeInventory">
    <div class="cssd_title">
      <ul class="cssd_menu">
        ...
        <router-link to="/inventory/disposableItemInbound" tag="li" v-if="UserInfo.ClinicType==1">
          <p>一次性物品入库</p>
        </router-link>
        <router-link to="/inventory/inboundRecord" tag="li" v-if="UserInfo.ClinicType==1">
          <p>入库记录</p>
        </router-link>
        <router-link to="/inventory/outboundRecord" tag="li" v-if="UserInfo.ClinicType==1">
          <p>出库记录</p>
        </router-link>
      </ul>
    ...
    <div class="cssd_table_center cssd_totalBar table_collapse table_unExpand">
      <div class="table_box">
        ...
        <el-collapse accordion @change="collapseChange">
          <el-collapse-item
            v-for="(item,index) in inventoryData.Products"
            :key="index"
            :name="index+''"
            :class="item.IsNotPrintBarCode?'collapseUnExpand':''"
          >
            ...
            <el-table
              v-if="!item.IsNotPrintBarCode&&!item.IsDisposableProduct"
              :data="item.Packages"
            >
    ...
    collapseChange(index) {
      if (
        ...
        (this.inventoryData.Products[index].Packages == "" ||
          this.inventoryData.Products[index].Packages === null) &&
        !this.inventoryData.Products[index].IsNotPrintBarCode
      ) {
        let url = `/api/inventory/HasBarCodePackagesForClinic/${this.UserInfo.ClinicId}/${this.inventoryData.Products[index].ProductId}`; //获取主科室的二次请求
        if (this.SubClinicId) {
