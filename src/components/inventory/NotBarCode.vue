      ...
      <div class="cssd_title_right">
        <p>
          ...
          >
            ...
            <el-option
              v-for="(item,index) in inventoryData.SubClinics"
              :key="index"
  ...
  methods: {
    changeClinicInventory(val){
      if(val==0){
        ...
        //子科室接口
        axios({url:`/api/Inventory/NotHasBarCode/${val}`}).then(res=>{
    ...
    //获取登录的主科室库存
    getClinicData(){
      axios({url:`/api/Inventory/NotHasBarCodeForClinic/${this.ClinicId}`}).then(res=>{
        if(res.data.Code == 200){
